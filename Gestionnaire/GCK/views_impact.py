"""Vues pour l'Académie d'Impact (chaque samedi matin des GCK — jeunes/jeunes professionnels)."""
...
from .models import BilanImpact
from .forms import BilanImpactForm
from .resources import BilanImpactResource
from .views import (
...
_SECTION_TITLE = "Académie d'Impact (GCK — Samedi)"
_TEMPLATE_PREFIX = "GCK/impact"


def _export(bilans, totaux, fmt, label):
    return _export_section_bilans(
        bilans, totaux, fmt,
        BilanImpactResource, 'impact_rapport',
...
def impact_list(request):
    bilans = BilanImpact.objects.all()
...
def add_impact(request):
    ...
    if request.method == 'POST':
        form = BilanImpactForm(request.POST, user=user)
        if form.is_valid():
            ...
            bilan = form.save()
            return redirect('impact_detail', pk=bilan.pk)
    else:
        form = BilanImpactForm(user=user)
...
@login_required(login_url='/membres/login/')
def impact_detail(request, pk):
    bilan = get_object_or_404(BilanImpact, pk=pk)
...
class EditImpact(RBACMixin, UpdateView):
    model = BilanImpact
    form_class = BilanImpactForm
    ...
    def get_success_url(self):
        return reverse_lazy('impact_detail', kwargs={'pk': self.object.pk})


# ─── Supprimer ────────────────────────────────────────────────────────────────

class DeleteImpact(AdminOnlyMixin, DeleteView):
    model = BilanImpact
    template_name = f'{_TEMPLATE_PREFIX}_delete.html'
    success_url = reverse_lazy('impact_list')


# ─── Rapport par District ─────────────────────────────────────────────────────

@login_required(login_url='/membres/login/')
def impact_district(request, dist_pk):
    eglise = get_object_or_404(Eglise, pk=dist_pk)
    bilans = BilanImpact.objects.filter(eglise=eglise)
    bilans = filter_by_rbac(request.user, bilans, 'bilan')
    bilans, start_date, end_date, mois, jour = _apply_date_filters(bilans, request.GET)
...
@login_required(login_url='/membres/login/')
def impact_groupe(request, group):
    groupe = get_object_or_404(Groupe, name=group)
    lbl = get_labels(groupe.region)
    bilans = BilanImpact.objects.filter(eglise__groupe=group)
    bilans = filter_by_rbac(request.user, bilans, 'bilan')
    bilans, start_date, end_date, mois, jour = _apply_date_filters(bilans, request.GET)
...
@login_required(login_url='/membres/login/')
def impact_region(request, regs):
    region = get_object_or_404(Region, name=regs)
    lbl = get_labels(region.name)
    bilans = BilanImpact.objects.filter(eglise__region=regs)
    bilans = filter_by_rbac(request.user, bilans, 'bilan')
    bilans, start_date, end_date, mois, jour = _apply_date_filters(bilans, request.GET)
